...
from .yaohud_music_handle import get_netease_music

netease_music = on_command("163mu")


# 用法 163mu [搜索名称] [选择的id | null]
...
async def netease_music_handle(args: Message = CommandArg()):
    ...
    if len(args_list) == 1:
        _res = await get_netease_music(msg_type="search", msg=args_list[0])
    ...
    if len(args_list) == 2:
        ...
        _res = await get_netease_music(msg_type="get", msg=args_list[0], n=int(args_list[1]))
        if _res == -1:
