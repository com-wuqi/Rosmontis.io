...
async def get_index_tts2(voice_txt: str, voice_from: str):
    ...
    async with _semaphore_ai, httpx.AsyncClient(timeout=120) as client:
    ...
    async with _semaphore_ai:
        async with httpx.AsyncClient(timeout=180) as client:
            ...
            try:
                ...
                file_png = store.get_plugin_cache_file(
                    f"yaohu_ai_picture-{time.time()}.jpg"
                )
...
async def get_weijin(txt: str) -> bool:
    ...
    async with _semaphore_ai, httpx.AsyncClient(timeout=60) as client:
